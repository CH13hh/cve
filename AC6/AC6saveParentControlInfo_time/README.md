...
Tenda AC6 V15.03.05.16 firmware has a buffer overflow vulnerability in the `saveParentControlInfo` function. The `sscanf(nptr, "%[^-]-%s", s1, s2);` function copies the `time` string content to `s1 and s2` without doing a boundary check, which will cause a buffer overflow and overwrite the memory area after the array, which may cause the program to crash, thereby triggering this security vulnerability.



![image-20250330233640193](README/image-20250330233640193.png)
...
![image-20250330234047860](README/image-20250330234047860.png)