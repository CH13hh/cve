...
Tenda AC6 V15.03.05.16 firmware has a buffer overflow vulnerability in the `formSetClientState` function. The `sprintf(s, "%d;%s;%s;%s", v2, v9, v10, v11);` function copies the contents of the `deviceId` string to `v9` without performing a boundary check. it will cause a buffer overflow and overwrite the memory area after the array, which may cause the program to crash, thus triggering this security vulnerability.

![image-20250330232035014](README/image-20250330232035014.png)
...
def execute_overflow(url, data):
    ...
    attack_params = {'deviceId':data,"limitEn":"1"}
...
![image-20250330195030422](README/image-20250330195030422.png)