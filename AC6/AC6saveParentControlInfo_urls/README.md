...
Tenda AC6 V15.03.05.16 firmware has a buffer overflow vulnerability in the `saveParentControlInfo` function. The ` strcpy((char *)s_3 + 80, src_1);` function copies the `urls` string content to `s_3` without doing a boundary check, which will cause a buffer overflow and overwrite the memory area after the array, which may cause the program to crash, thereby triggering this security vulnerability.



![image-20250330234848417](README/image-20250330234848417.png)
...
![image-20250330235119468](README/image-20250330235119468.png)